...
import { FaUserPlus, FaCalendarCheck, FaClipboardList, FaFileMedical, FaChevronRight, FaLock } from 'react-icons/fa';
...
const enrollmentSchema = [
  {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://rtms-behandeling.nl/" },
      { "@type": "ListItem", "position": 2, "name": "Intake aanvragen", "item": "https://rtms-behandeling.nl/aanmelden/" }
    ]
  },
  {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "mainEntity": [
      { "@type": "Question", "name": "Wordt de intake vergoed?", "acceptedAnswer": { "@type": "Answer", "text": "Ja, de intake maakt deel uit van de Specialistische GGZ en wordt vergoed door uw basisverzekering (rekening houdend met uw eigen risico), mits u een geldige verwijsbrief heeft." } },
      { "@type": "Question", "name": "Is er een wachtlijst?", "acceptedAnswer": { "@type": "Answer", "text": "Wij streven naar een minimale wachttijd. Gemiddeld kunt u binnen 1 à 2 weken terecht voor een officieel intakegesprek." } },
      { "@type": "Question", "name": "Kan ik eerst een kennismaking krijgen?", "acceptedAnswer": { "@type": "Answer", "text": "Zeker. We kunnen altijd eerst een telefonisch informatief gesprek inplannen om uw eerste vragen te beantwoorden." } }
    ]
  }
];
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden">
            ...
            {status === 'sent' ? (
              <div className="p-10 text-center space-y-4">
                ...
                <h3 className="text-2xl font-bold text-[#2b2e4a]">Uw aanvraag is verzonden!</h3>
                <p className="text-slate-600">Wij nemen binnen 1 werkdag contact met u op voor een telefonische kennismaking.</p>
              </div>
            ) : (
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" name="firstName" value={form.firstName} onChange={handleChange} required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-[#33C5F3] outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Achternaam *</label>
                    <input type="text" name="lastName" value={form.lastName} onChange={handleChange} required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-[#33C5F3] outline-none" />
                  </div>
                </div>
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">E-mailadres *</label>
                    <input type="email" name="email" value={form.email} onChange={handleChange} required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-[#33C5F3] outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Telefoonnummer *</label>
                    <input type="tel" name="phone" value={form.phone} onChange={handleChange} required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-[#33C5F3] outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 pt-4">
                ...
                <div>
                   ...
                   <select name="complaint" value={form.complaint} onChange={handleChange} required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-[#33C5F3] outline-none bg-white">
                ...
                <div>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <label className="flex items-center gap-3 p-4 border rounded-xl cursor-pointer hover:border-[#33C5F3] transition">
                         <input type="radio" name="location" value="heiloo" checked={form.location === 'heiloo'} onChange={handleChange} required className="w-5 h-5 text-[#33C5F3] focus:ring-[#33C5F3]" />
                         <span className="font-medium">Heiloo</span>
                      </label>
                      <label className="flex items-center gap-3 p-4 border rounded-xl cursor-pointer hover:border-[#33C5F3] transition">
                         <input type="radio" name="location" value="schiphol" checked={form.location === 'schiphol'} onChange={handleChange} className="w-5 h-5 text-[#33C5F3] focus:ring-[#33C5F3]" />
              ...
              <div className="space-y-6 pt-4">
                ...
                <div className="flex items-center gap-4">
                   <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" name="hasReferral" checked={form.hasReferral} onChange={handleChange} className="w-5 h-5 rounded border-slate-300 text-[#33C5F3] focus:ring-[#33C5F3]" />
              ...
              {status === 'error' && (
                <div className="bg-red-50 text-red-700 p-4 rounded-xl text-sm">
                  Er is iets misgegaan bij het verzenden. Probeer het later opnieuw of neem telefonisch contact op.
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-5 bg-[#33C5F3] text-white text-xl font-bold rounded-xl hover:bg-[#2b2e4a] transition-all shadow-xl shadow-cyan-100 mt-4 disabled:opacity-50"
              >
                {status === 'sending' ? 'Bezig met verzenden...' : 'Aanvraag Afronden'}
              </button>
